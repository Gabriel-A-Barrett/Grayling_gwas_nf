...
awk -F'[ ]' 'NR==FNR{a[$1];next} $1 in a {print $2}' ${ENV}_vcf.indv $LFMM > ${ENV}_m_lfmm.env

# convert vcf to baypass
cat ${ENV}_${VCF} | perl $baseDir/vcf2baypass.pl $GROUP ${ENV}.frq
